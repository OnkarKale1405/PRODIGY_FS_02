...
const parseNumber = (value) => parseInt(value) || 0;
...
const registerUser = asyncHandler(async (req, res) => {
    ...
    if (
        [username, email, role_id, password].some((field) => field?.trim() === "")
    ) {
...
// this is used while verifing email, if it is the actual email of the registering user or not 
...
const forgotPasswordRequest = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, {}, "Password reset mail sent successfully"));
...
const resetForgottenPassword = asyncHandler(async (req, res) => {
    ...
    if (!verifiedToken) {
        throw new ApiError(402, "password reset token is required");
    }
    if (!newPassword) {
